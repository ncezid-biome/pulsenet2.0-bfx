...
include {CBOT_TOXINTYPER}            from '../modules/cbot_toxintyper'
include {FIND_GENES}                 from '../modules/find_genes'
include {PLASMIDFINDER}              from '../modules/plasmidfinder'
include {COLLECT_RESULTS_CBOT}       from '../modules/collect_results'
...
workflow CBOT_WORKFLOW {
    
    // [sample, assembly, "CBOT", MasterOutput, CRAM, read1, read2, "Clostridium", fastp results, narst_assembly]
    take: data
    ...
    CBOT_TOXINTYPER(assembly_ani_genotyping, params.cbot_toxintyping_db)

    latest_master = ALLELE_FILTERING.out.master.join(qc_end)

    //genotype_master_input = [sampleID, latestMaster(ani), 
    //        AMRFinderResultsRaw.json(findgenes),
    //        outputs.json(PF), plasmidfinder.json(PF), 
    //        outputs.json(MLST)],
    //        
    genotype_master_input = latest_master
        ...
        .join(CBOT_TOXINTYPER.out.master)

    COLLECT_RESULTS_CBOT(genotype_master_input, params.masterOutScript, params.genotypeFormatter)

    adx_input = COLLECT_RESULTS_CBOT.out.adx_input.join(fastp).join(removeFromEnd(ALLELE_FILTERING.out.standard_calls, 2))
