...
//load modules
include {DEINTERLEAVE}                  from './modules/deinterleave'
...
include {FETCH_READ_PATHS}              from './modules/fetch_paths'
...
workflow {
    if (params.source=="custom") {
        fastq_dir = Channel
                ...
                .map { row -> tuple( row.sample, row.file_path, params.initialMaster) }

        FETCH_READ_PATHS(fastq_dir)
        fastq_path_temp = FETCH_READ_PATHS.out.fastqPath
    } 
    else if (params.source == "ncbi"){
        fastq_path_temp = Channel
                .fromPath(params.reads)
                .splitCsv(header:true, strip:true)
                .map { row -> tuple( row.sample, row.file_path, params.initialMaster) } | DEINTERLEAVE
        fastq_path_temp = filterQC(DEINTERLEAVE.out.reads)
    }
    else { 
