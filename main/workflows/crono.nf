...
include {COLLECT_RESULTS_CRONO}       from '../modules/collect_results'
...
workflow CRONO_WORKFLOW {
    
    // [sample, assembly, "CRONO", MasterOutput, CRAM, read1, read2, fastp results, "Cronobacter"]
    take: data
    ...
    COLLECT_RESULTS_CRONO(genotype_master_input, params.masterOutScript, params.genotypeFormatter)
    
    adx_input = COLLECT_RESULTS_CRONO.out.adx_input.join(fastp).join(removeFromEnd(ALLELE_FILTERING.out.standard_calls, 2))
