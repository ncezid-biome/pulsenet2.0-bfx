...
include {VIBRIO_SEROTYPER}          from '../modules/vibrio_typer'
include {VIBRIO_VIRULENCE_FINDER}   from '../modules/vibrio_typer'
include {COLLECT_RESULTS_VIBRIO}    from '../modules/collect_results'
...
workflow VIBRIO_WORKFLOW {
    
    // [sample, assembly, "VIBRIO", MasterOutput, CRAM, read1, read2, "Vibrio", fastp results, narst_assembly]
    take: data
    ...
    //add the pipeline output that contains the species which is required for find genes 
    //corrAssem_fullGenus = [sampleID, assembly[corrAssem], Genus(midas), species]
    ...
    VIBRIO_SEROTYPER(assembly)
    VIBRIO_VIRULENCE_FINDER(assembly)    
    ...
    //genotype_master_input = [sampleID, latestMaster(allele_naming), outputs.json(findgenes), AMRFinderResultsRaw.json(findgenes),
    ...
    genotype_master_input = latest_master
        ...
        .join(VIBRIO_SEROTYPER.out.master)
        .join(VIBRIO_VIRULENCE_FINDER.out.master)

    //COLLECT_GENOTYPE_RESULTS(genotype_master_input, params.masterOutScript, params.genotypeFormatter)
    COLLECT_RESULTS_VIBRIO(genotype_master_input, params.masterOutScript, params.genotypeFormatter)

    //adx_input = [masterOut(collect_Results), genotypeResults(collect_Results), fastpReport(prepReads), outputs.json(alleleNaming)]
    adx_input = COLLECT_RESULTS_VIBRIO.out.adx_input.join(fastp).join(removeFromEnd(ALLELE_FILTERING.out.standard_calls, 2))
