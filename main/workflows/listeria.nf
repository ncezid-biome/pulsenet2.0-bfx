...
include {COLLECT_RESULTS_LISTERIA}   from '../modules/collect_results'
...
workflow LISTERIA_WORKFLOW {
    
    // [sample, assembly, "LISTERIA", MasterOutput, CRAM, read1, read2, "Listeria", fastp results, narst_assembly]
    take: data
    ...
    COLLECT_RESULTS_LISTERIA(genotype_master_input, params.masterOutScript, params.genotypeFormatter)

    adx_input = COLLECT_RESULTS_LISTERIA.out.adx_input.join(fastp).join(removeFromEnd(ALLELE_FILTERING.out.standard_calls, 2))
