...
class GenotypeHit(object):
    ...
    def from_blast(line):
        ...
        hit.coverage = float((hit._query_stop-hit._query_start+1)/abs(hit._reference_stop-hit.reference_start+1))
    ...
    def identify_amplicons(self) -> None:
        ...
        for primer_hit, hits in good_primer_hits.items():
            for hit1, hit2 in combinations(hits, 2):
                ...
                target_lengths = []
                for primer_set in self.primers[primer_hit]:
                    if (
                        hit1.query_id in {primer_set["forward_primer_id"], primer_set["reverse_primer_id"]}
                        and hit2.query_id in {primer_set["forward_primer_id"], primer_set["reverse_primer_id"]}
                    ):
                        target_lengths.append(primer_set["expected_length"])
